...
class BackendApi {
  final String baseUrl; // ex.: http://localhost:8787
  BackendApi({required this.baseUrl});

  Uri _u(String path, [Map<String,String?> q = const {}]) => Uri.parse('$baseUrl$path').replace(queryParameters: {
    ...q,
  ...
  Future<Map<String,dynamic>> askAssistant(String query) async {
    final res = await http.post(_u('/api/assistant'), headers: {'Content-Type':'application/json'}, body: jsonEncode({'query': query}));
    if (res.statusCode!=200) throw Exception('HTTP ${res.statusCode}: ${res.body}');
    return jsonDecode(res.body) as Map<String,dynamic>;
  }
}
